...
##计算费用
def calculate_audit_fee(service_item, amount, verification_type=None, years=None, hours_by_level=None, return_details=False):
    """
    根据服务项目计算审计收费（含计算明细）
    """
...
def main(verification_type=None, years=None, hours_by_level=None, company_name="公司名称", service_category="audit",
         service_item="财务报表审计", amount=0, amount_type="asset", special_requirements="", show_details=True) -> dict:
    """
    主函数：计算审计服务收费

    参数：
    - verification_type: 验资类型（"货币"或"其他"）
    - years: 年数（用于合并分立清算审计和经济责任审计）
    - hours_by_level: 计时收费的各级别工时字典
    - company_name: 客户名称
    ...
    if hours_by_level:
        result["hours_breakdown"] = hours_by_level
